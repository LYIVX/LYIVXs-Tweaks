...
public final class CraftingShapelessCategory implements RecipeCategory<ShapelessRecipe> {
    private static final ResourceLocation ID = ResourceLocation.fromNamespaceAndPath("minecraft", "crafting_shapeless");
    ...
    @Override
    public String titleTranslationKey() { return "ls_tweaks.category.crafting_shapeless"; }
    ...
    public RecipeDisplay buildDisplay(ShapelessRecipe recipe) {
        try {
    ...
    private RecipeDisplay buildDisplaySimple(ShapelessRecipe recipe) {
        ...
        var ings = net.lyivx.ls_tweaks.recipes.util.RecipeUtil.ingredientsFirstStacks(recipe);
        int index = 0;
        for (int r = 0; r < 3; r++) {
            for (int c = 0; c < 3; c++) {
                int x = c * 18;
                int y = r * 18;
                ItemStack stack = index < ings.size() ? ings.get(index) : ItemStack.EMPTY;
        ...
        builder.addText(Widgets.AddText.literal(50, 0, "S"));
