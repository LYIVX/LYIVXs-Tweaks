...
public class GridSlotWidget extends SlotWidget {
    ...
    public void render(GuiGraphics g, Font font, int baseX, int baseY, int mouseX, int mouseY) {
        ...
        int outOffset = 0;
        int total = (items != null && !items.isEmpty()) ? items.size() : (slotWidget == null ? 0 : slotWidget.getStackListSize());
        for (int r = 0; r < rows; r++) {
            ...
            for (int c = 0; c < cellsInRow; c++) {
                ...
                SlotWidget cell = (slotWidget == null)
                        ? (withBackground
                            ? (asOutput ? SlotWidget.output(cx - baseX, cy - baseY, ItemStack.EMPTY, idx) : SlotWidget.input(cx - baseX, cy - baseY, ItemStack.EMPTY, idx))
                            : (asOutput ? SlotWidget.outputNoBackground(cx - baseX, cy - baseY, ItemStack.EMPTY, idx) : SlotWidget.inputNoBackground(cx - baseX, cy - baseY, ItemStack.EMPTY, idx)))
                        : slotWidget.copyForGridCell(cx - baseX, cy - baseY, idx);
