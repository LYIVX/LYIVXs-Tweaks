...
public final class RefillClient {
    ...
    private static void onClientTick() {
        ...
        if (!QolConfig.refill().enabled) return;
        if (!QolConfig.refill().workOnContainerScreens && mc.screen != null) {
            return;
        }
        ...
        if (becameEmpty || changedTypeOrComponents) {
            var payload = new QolNet.RefillC2SPayload(slot, lastSnapshot.copy(), QolConfig.refill().strictNbt);
